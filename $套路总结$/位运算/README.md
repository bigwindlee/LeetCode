...
|371. 两整数之和|异或操作：从二进制层面实现加法；与操作左移提取进位；异或操作（模2加法）；|
|461. 汉明距离|异或/LOWBIT：先做异或，然后统计结果中`1`比特的个数；|
|476. 数字的补数|从高比特位到低比特位的方向，把连续的`1`比特清零；|
|477. 汉明距离总和|位操作：第i比特为1的元素个数 * 第i比特为0的元素个数|
|[645. 错误的集合][github-leetcode-0645]|LOWBIT：根据lowbit把大数组分成2组，分别做异或；|
|1684. 统计一致字符串的数目|位运算：小写字母状态压缩|
|1720. 解码异或后的数组|异或操作：湮灭相同|
|1863. 找出所有子集的异或总和再求和|位运算：子集状态压缩|
|2032. 至少在两个数组中出现的值|位运算：用mask[n]的低三位比特描述n在三个数组中的出现情况；|
...
[github-leetcode-0645]: ../../0645.%20Set%20Mismatch/0645_findErrorNums.h
