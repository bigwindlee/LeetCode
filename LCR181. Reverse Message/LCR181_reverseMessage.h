...
public:
    string reverseMessage(string message)
    {
        int n = message.size();
        string ans;
        int right = n - 1; // 用数组下标作为指针，从尾部向头部逆向扫描。
        while (right >= 0) {
            ...
            int left = right; // left和right合作，界定一个单词。
            while (left >= 0 && message[left] != ' ') {
                --left;
            }
            ans += message.substr(left + 1, right - left);
            ans += ' ';
            right = left; // 迭代：right从left开始继续向左寻找下一个非空字符
        }
        // 如果message全是空格，那么ans为空。所以必须先判断一下ans是否为空再弹出末尾的空格。
        if (ans.size() > 0) {
            ans.pop_back();
        }
        return ans;
